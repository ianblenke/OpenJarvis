...
class TestChannelImportError:
    ...
    @pytest.mark.spec("REQ-channels.protocol.lifecycle")
    def test_connect_import_error(self, cls, key, lib_mod, pip_pkg, monkeypatch):
...
class TestViberChannel:
    ...
    def test_env_fallback(self, monkeypatch):
        ...
        monkeypatch.setenv("VIBER_BOT_NAME", "TestBot")
        ch = ViberChannel()
...
class TestMastodonChannel:
    @pytest.mark.spec("REQ-channels.protocol.lifecycle")
    def test_env_fallback(self, monkeypatch):
        monkeypatch.setenv("MASTODON_API_BASE_URL", "https://m.social")
...
class TestXMPPChannel:
    ...
    def test_env_fallback(self, monkeypatch):
        ...
        monkeypatch.setenv("XMPP_PASSWORD", "pass")
        monkeypatch.setenv("XMPP_SERVER", "xmpp.example.com")
...
class TestRocketChatChannel:
    ...
    def test_env_fallback(self, monkeypatch):
        ...
        monkeypatch.setenv("ROCKETCHAT_USER", "bot")
        monkeypatch.setenv("ROCKETCHAT_PASSWORD", "pass")
    ...
    @pytest.mark.spec("REQ-channels.protocol.lifecycle")
    def test_token_auth_env(self, monkeypatch):
        monkeypatch.setenv("ROCKETCHAT_URL", "https://rc.example.com")
        monkeypatch.setenv("ROCKETCHAT_AUTH_TOKEN", "tok")
        monkeypatch.setenv("ROCKETCHAT_USER_ID", "uid")
        ch = RocketChatChannel()
...
class TestZulipChannel:
    ...
    def test_env_fallback(self, monkeypatch):
        ...
        monkeypatch.setenv("ZULIP_SITE", "https://z.com")
        ch = ZulipChannel()
    ...
    @pytest.mark.spec("REQ-channels.protocol.lifecycle")
    def test_zuliprc_env(self, monkeypatch):
...
class TestNostrChannel:
    @pytest.mark.spec("REQ-channels.protocol.lifecycle")
    def test_env_fallback(self, monkeypatch):
        monkeypatch.setenv("NOSTR_PRIVATE_KEY", "aa" * 32)
