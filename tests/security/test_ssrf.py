...
import openjarvis.security.ssrf as _ssrf_mod
from openjarvis.security.ssrf import _check_ssrf_python, check_ssrf, is_private_ip


class TestIsPrivateIp:
    @pytest.mark.spec("REQ-security.ssrf.private-ip")
...
class TestCheckSsrf:
    ...
    @pytest.mark.spec("REQ-security.ssrf.metadata")
    @pytest.mark.spec("REQ-security.ssrf.private-ip")
    ...
    @pytest.mark.spec("REQ-security.ssrf.private-ip")
    def test_allows_normal_urls(self, monkeypatch):
        # Use Python impl so we can stub DNS resolution
        monkeypatch.setattr(
            _ssrf_mod.socket, "getaddrinfo",
    ...
    @pytest.mark.spec("REQ-security.ssrf.private-ip")
    def test_blocks_localhost_url(self, monkeypatch):
        monkeypatch.setattr(
            _ssrf_mod.socket, "getaddrinfo",
    ...
    @pytest.mark.spec("REQ-security.ssrf.private-ip")
    def test_blocks_private_ip_url(self, monkeypatch):
        monkeypatch.setattr(
            _ssrf_mod.socket, "getaddrinfo",
    ...
    @pytest.mark.spec("REQ-security.ssrf.dns-rebind")
    def test_dns_failure_allowed(self, monkeypatch):
        ...
        monkeypatch.setattr(_ssrf_mod.socket, "getaddrinfo", _raise)
